...
public class HibernateConnectionConfig {
    ...
    public HibernateConnectionConfig(
            final @Value("${db.driver}") String dbDriver,
            final @Value("${db.url}") String dbUrl,
            final @Value("${db.username}") String dbUsername,
            final @Value("${db.password}") String dbPassword,
            final @Value("${config.hibernate.hbm2ddl.auto}") String hibernateDdlAuto,
            final @Value("${config.hibernate.dialect}") String hibernateDialect) {
    ...
    public DataSource dataSource() {
        return DataSourceBuilder.create().driverClassName(this.dbDriver).url(this.dbUrl).username(this.dbUsername)
